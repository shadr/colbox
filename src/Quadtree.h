...
class Quadtree {
  static_assert(std::is_convertible_v<std::invoke_result_t<GetBox, const T &>,
                                      Box<Float>>,
...
public:
  Quadtree(const Box<Float> &box, const GetBox &getBox = GetBox(),
      ...
      : mBox(box), mRoot(std::make_unique<Node>()), mGetBox(getBox),
        mEqual(equal) {}
...
private:
  ...
  Box<Float> computeBox(const Box<Float> &box, int i) const {
    ...
    // Norst East
    case 1:
      return Box<Float>(Vector2<Float>(origin.x + childSize.x, origin.y),
                        childSize);
    // South West
    case 2:
      return Box<Float>(Vector2<Float>(origin.x, origin.y + childSize.y),
                        childSize);
    // South East
    case 3:
  ...
  void add(Node *node, std::size_t depth, const Box<Float> &box,
    ...
    if (isLeaf(node)) {
      ...
      if (i != -1)
        add(node->children[static_cast<std::size_t>(i)].get(), depth + 1,
